...
import { MaxUint256, sqrt, Fraction, Rounding, Percent, TradeType, sortedInsert } from '@uniswap/sdk-core';
...
var TickMath = /*#__PURE__*/function () {
  ...
  TickMath.getTickAtSqrtRatio = function getTickAtSqrtRatio(sqrtRatioX96_) {
    !(JSBI.greaterThanOrEqual(sqrtRatioX96_, TickMath.MIN_SQRT_RATIO) && JSBI.lessThan(sqrtRatioX96_, TickMath.MAX_SQRT_RATIO)) ? process.env.NODE_ENV !== "production" ? invariant(false, 'SQRT_RATIO') : invariant(false) : void 0;
...
function sortBefore(a, b) {
  !(a !== b) ? process.env.NODE_ENV !== "production" ? invariant(false, 'ADDRESSES') : invariant(false) : void 0;
  if (a.includes('::')) {
...
var Trade = /*#__PURE__*/function () {
  ...
  _proto.maximumAmountIn = function maximumAmountIn(slippageTolerance, amountIn) {
    ...
    console.log('maximumAmountIn', this.tradeType, TradeType.EXACT_INPUT);
      ...
      return _regeneratorRuntime().wrap(function _callee3$(_context3) {
        while (1) switch (_context3.prev = _context3.next) {
          case 0:
            ...
            minimumAmounts = position.mintAmountsWithSlippage(new Percent(slippageTolerance.numerator, slippageTolerance.denominator));
      ...
      return _regeneratorRuntime().wrap(function _callee4$(_context4) {
        while (1) switch (_context4.prev = _context4.next) {
          case 0:
            ...
            minimumAmounts = position.mintAmountsWithSlippage(new Percent(slippage.numerator, slippage.denominator));
            amount0Min = minimumAmounts.amount0.toString();
            amount1Min = minimumAmounts.amount1.toString();
            typeArgs = [position.pool.token0.address, position.pool.token1.address, this.moduleAddress + getFeeType(position.pool.fee)];
            moveCallTx = {
      ...
      return _regeneratorRuntime().wrap(function _callee5$(_context5) {
        while (1) switch (_context5.prev = _context5.next) {
          case 0:
            !JSBI.greaterThan(position.liquidity, ZERO) ? process.env.NODE_ENV !== "production" ? invariant(false, 'ZERO_LIQUIDITY') : invariant(false) : void 0;
            // adjust for slippage
            minimumAmounts = position.burnAmountsWithSlippage(new Percent(slippage.numerator, slippage.denominator));
            amount0Min = minimumAmounts.amount0.toString();
            amount1Min = minimumAmounts.amount1.toString();
            typeArgs = [position.pool.token0.address, position.pool.token1.address, this.moduleAddress + getFeeType(position.pool.fee)];
            moveCallTx = {
...
var SwapRouter = /*#__PURE__*/function () {
  ...
  _proto.createSwapTx = function createSwapTx(coinStores, coinIns, trade, options, gasBudget) {
    ...
    !trade.inputAmount.currency.wrapped.equals(tokenIn) ? process.env.NODE_ENV !== "production" ? invariant(false, 'TOKEN_IN_DIFF') : invariant(false) : void 0;
    !trade.outputAmount.currency.wrapped.equals(tokenOut) ? process.env.NODE_ENV !== "production" ? invariant(false, 'TOKEN_OUT_DIFF') : invariant(false) : void 0;
    ...
    var slippage = new Percent(options.slippageTolerance.numerator, options.slippageTolerance.denominator);
...
export { CurrencyAmount, EventFetcher, EventType, FeeAmount, FullMath, LiquidityMath, NoTickDataProvider, Payments, Pool, Position, PositionLibrary, PositionManager, Price, Route, SimpleTickDataProvider, SqrtPriceMath, StateFetcher, SuiCoin, SwapMath, SwapRouter, TICK_SPACINGS, Tick, TickLibrary, TickList, TickListDataProvider, TickMath, Trade, TypeTagSerializer, bnToBigInt, decimalToHexString, encodeSqrtRatioX96, feeTypeToFeeAmount, getFeeType, getModuleAddress, hexToBN, i128ToBigInt, i256ToBigInt, i64ToNumber, isSorted, maxLiquidityForAmounts, mostSignificantBit, nearestUsableTick, parseTypeFromStr, priceToClosestTick, setModuleAddress, sleep, sortBefore, sortTypes, structTagToString, subIn256, tickToPrice, toFullyQualifiedName, toHex, toUTF8Array, tradeComparator, tryCallWithTrial, typeInfoToQualifiedName, validateAndParseAddress };
//# sourceMappingURL=sui-sdk.esm.js.map
